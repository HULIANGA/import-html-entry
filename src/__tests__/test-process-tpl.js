...
test('test process-tpl', () => {
  ...
  expect(
    template.indexOf(
      genScriptReplaceSymbol('http://kuitos.me/test-async.js', true)
    ) !== -1
  ).toBeTruthy()
  expect(
    template.indexOf(genLinkReplaceSymbol('http://kuitos.me/umi.css')) !== -1
  ).toBeTruthy()
  expect(
    template.indexOf(
      genLinkReplaceSymbol(
        'http://kuitos.me/escape-character%C2%A0.css?a=&&b=2'
      )
    ) !== -1
  ).toBeTruthy()
  expect(
    template.indexOf(genScriptReplaceSymbol('http://kuitos.me/umi.js')) !== -1
  ).toBeTruthy()
  expect(
    template.indexOf(
      genScriptReplaceSymbol('http://kuitos.me/escape-character&.js')
    ) !== -1
  ).toBeTruthy()
  expect(
    template.indexOf(genScriptReplaceSymbol('http://kuitos.me/comment.js')) !==
			-1
  ).toBeTruthy()
  expect(
    template.indexOf(genScriptReplaceSymbol('http://kuitos.me/main-es5.js')) !==
			-1
  ).toBeTruthy()
  expect(
    template.indexOf('<script src="/test-type.json" type="test"></script>') !==
			-1
  ).toBeTruthy()
  expect(
    template.indexOf('<script type=systemjs-importmap>{"a": 1}</script>') !==
			-1
  ).toBeTruthy()
  ...
  expect(
    template.indexOf(genLinkReplaceSymbol('/a3-ie6-polyfill.js', true)) !== -1
  ).toBeTruthy()
  // prefetch 资源直接被 ignore
  expect(
    template.indexOf('<link rel="prefetch" href="/a3-ie6-polyfill.js">') === -1
  ).toBeTruthy()
  // type="module" 资源被 ignore
  expect(
    template.indexOf(genModuleScriptReplaceSymbol('/main-es2015.js', false)) ===
			-1
  ).toBeTruthy()

  const { styles, template: template2 } = processTpl(
    tpl,
    'http://kuitos.me/cdn/'
  )
  ...
  expect(
    template2.indexOf(genLinkReplaceSymbol('http://kuitos.me/umi.css')) !== -1
  ).toBeTruthy()
})

test('test ignore js or css', () => {
  const tpl = readFileSync(require.resolve('./fixtures/ignore-js-css.html'), {
    encoding: 'utf-8'
  })
  ...
  expect(
    template.indexOf(genIgnoreAssetReplaceSymbol('style file')) !== -1
  ).toBeTruthy()
  ...
  expect(
    template.indexOf(
      genIgnoreAssetReplaceSymbol('http://kuitos.me/umi.css')
    ) !== -1
  ).toBeTruthy()

  expect(
    template.indexOf(
      genScriptReplaceSymbol('//cdn.jsdelivr.net/npm/vue@2.6.10/dist/vue.js')
    ) === -1
  ).toBeTruthy()
  expect(
    template.indexOf(genScriptReplaceSymbol('http://kuitos.me/polyfill.js')) ===
			-1
  ).toBeTruthy()
  expect(
    template.indexOf(
      genIgnoreAssetReplaceSymbol('http://kuitos.me/polyfill.js')
    ) !== -1
  ).toBeTruthy()
  expect(
    template.indexOf(genIgnoreAssetReplaceSymbol('js file')) !== -1
  ).toBeTruthy()
})
...
test('test resource with no quotation marks', () => {
  ...
  expect(
    template.indexOf(
      '<link rel="prefetch" href=http://kuitos.me/a3-ie6-polyfill.js>'
    ) === -1
  ).toBeTruthy()
  // type="module" 资源被 ignore
  expect(
...
test('should work with huge html content', () => {
  const hugeHtmlContent = readFileSync(
    require.resolve('./fixtures/huge-content.html'),
    'utf-8'
  )
