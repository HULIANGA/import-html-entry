...
test('test custome fetch importHTML', async (done) => {
  ...
  const importHTML = jest.fn().mockImplementation((url, { fetch }) => {
    return {
      getExternalScripts: () =>
        rawGetExternalScripts(['http://kuitos.me/index.js'], fetch),
      getExternalStyleSheets: () =>
        rawGetExternalStyleSheets(['http://kuitos.me/index.css'], fetch)
    }
  })

  const { getExternalScripts, getExternalStyleSheets } = await importHTML(
    'http://kuitos.me',
    { fetch: (url) => fetch(url, { headers: mockHeaders }) }
  )
  ...
  expect(
    fetch.mock.calls.reduce((pre, cur) => {
